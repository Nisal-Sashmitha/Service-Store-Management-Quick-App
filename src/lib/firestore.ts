...
export function watchServices(
  onChange: (services: ServiceCatalogItem[]) => void,
  onError?: (err: Error) => void
): Unsubscribe {
  const db = getDb();
...
export function watchTicket(
  ...
  return onSnapshot(
    ...
    (snap) =>
      onChange(
        snap.exists()
          ? ({ id: snap.id, ...(snap.data() as Omit<Ticket, "id">) } as Ticket)
          : null
      ),
    (err) => onError?.(err as Error)
  );
...
export function watchTicketServiceItems(
  ...
  const db = getDb();
  const q = query(
    collection(db, "tickets", ticketId, "serviceItems"),
...
export function watchTicketActions(
  ...
  const db = getDb();
  const q = query(
    collection(db, "tickets", ticketId, "actions"),
...
export async function createTicket(input: {
  ...
  for (const item of input.serviceItems) {
    ...
    const appointmentDate = parseDateInputValue(
      item.appointmentDateTimeInput ?? ""
    );
    const appointmentRef = doc(
      db,
      "appointments",
      `${ticketRef.id}__${item.serviceId}`
    );
    batch.set(itemRef, {
...
export async function updateTicket(input: {
  ...
  for (const item of input.serviceItems) {
    ...
    const appointmentRef = doc(
      db,
      "appointments",
      `${input.ticketId}__${item.serviceId}`
    );
    if (appointmentDate) {
      batch.set(
        appointmentRef,
  ...
  for (const action of input.actions) {
    ...
    if (!dueDate) continue;
    const actionId =
      action.id ?? doc(collection(db, "tickets", input.ticketId, "actions")).id;
  ...
  for (const prevId of input.previousActionIds) {
    if (!nextActionIds.includes(prevId))
      batch.delete(doc(db, "tickets", input.ticketId, "actions", prevId));
  }
...
export async function deleteCatalogItem(
  collectionName: "employees" | "services",
  id: string
): Promise<void> {
  const db = getDb();
...
export async function ensureCatalogSeed(
  ...
  const existing = new Set(
    snap.docs.map((d) =>
      String((d.data() as { name?: string }).name ?? "").toLowerCase()
    )
  );
...
export async function updateLedgerEntry(input: {
  ...
  const amount = Number(input.amount);
  if (!Number.isFinite(amount) || amount < 0)
    throw new Error("Amount must be a number (0 or more).");
  ...
  if (input.type === "INCOME") {
    if (!input.serviceId || !input.serviceName)
      throw new Error("Service is required for income.");
...
export async function completeServiceAndRecordIncome(input: {
  ...
  batch.update(
    doc(db, "tickets", input.ticketId, "serviceItems", input.serviceId),
    {
      isCompleted: true,
      completedAt,
      updatedAt: serverTimestamp(),
  ...
  batch.update(doc(db, "tickets", input.ticketId), {
    updatedAt: serverTimestamp(),
  });
...
export async function createBillAndIncomeEntries(input: {
  ...
  if (!lines.length)
    throw new Error("Add at least 1 service with a valid quantity and amount.");
  ...
  const totalAmount = lines.reduce(
    (sum, l) => sum + l.unitAmount * l.quantity,
    0
  );
